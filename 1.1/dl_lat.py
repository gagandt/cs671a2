...
# Inputs
x = tf.placeholder(tf.float32, shape=(128, 28, 28, 3), name='input_x')
y1 =  tf.placeholder(tf.float32, shape=(128, 2), name='angle_y')
y2 =  tf.placeholder(tf.float32, shape=(128, 2), name='angle_y')
y3 =  tf.placeholder(tf.float32, shape=(128, 2), name='angle_y')
y4 =  tf.placeholder(tf.float32, shape=(128, 12), name='angle_y')
...
epochs = 10
batch_size = 128
no_of_batches = int(96000/batch_size)
...
y_leng = to_categorical(y_leng)
y_widt = to_categorical(y_widt)
y_colo = to_categorical(y_colo)
...
def load_preprocess_training_batch(batch_number):
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        n_batches = no_of_batches
        
        print(n_batches)

        for batch_i in range(0, n_batches):
            #for batch_features, length_labels, width_labels, color_labels, angle_labels in load_preprocess_training_batch(batch_i):
            ...
            print_stats(sess, X_train[batch_number] , y_leng[batch_number] , y_widt[batch_number] , y_colo[batch_number] , y_angl[batch_number], cost, accuracy)
